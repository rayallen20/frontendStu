    ...
    <div class="singer-cpn">
        ...
        <div class="singer-list">
            <div class="singer-item" v-for="singer in singers" :key="singer.id">
                ...
                <p>
                    <RouterLink class="singer-link" :to="{name: 'discoverSinger', params: {singerId: singer.id}}">{{singer.name}}</RouterLink>
                </p>
            </div>
        </div>

        <div class="content" v-if="hasSingerId()">
            <img :src="currentSinger !== null ? currentSinger.bigImg : ''" alt="">
        </div>
    </div>
</template>
